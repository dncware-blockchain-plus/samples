...
var { adminWalletPassword, peerURL, chainID, domain: defaultDomain } = await import('../lib/load-config.mjs');
var { adminWalletJSON } = await import('../lib/load-wallet.mjs');
var adminWallet = await api.unlockWalletFile(await api.parseWalletFile(adminWalletJSON), adminWalletPassword);
...
async function createObjectF(type, name, domain = defaultDomain) {
...
async function deploySmartContract(argtypes, func, options) {
    ...
    var { name, domain, replacers = [] } = options || {};
    ...
    var id = await createObjectF('contract', name, domain);
